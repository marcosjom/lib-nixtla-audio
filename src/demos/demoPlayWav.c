...
int main(int argc, const char * argv[]){
    ...
    STNixDemoPlayWav demo = STNixDemoPlayWav_Zero;
    if(!NixDemosCommon_init(&common)){
        NIX_PRINTF_ERROR("ERROR, NixDemosCommon_init failed.\n");
        return -1;
    } else {
        if(!NixDemoPlayWav_init(&demo, &common)){
            NIX_PRINTF_ERROR("ERROR, NixDemoPlayWav_init failed.\n");
            return -1;
        } else {
            //cycle
            {
                ...
                while(secsAccum < NIX_DEMO_PLAY_SECS_TO_EXIT){
                    ...
                    if(msAccum >= 1000){
                        ...
                        NIX_PRINTF_INFO("%u/%u secs running\n", secsAccum, NIX_DEMO_PLAY_SECS_TO_EXIT);
                        msAccum %= 1000;
                    }
                }
            }
            NixDemoPlayWav_destroy(&demo);
        }
